...
//	}
    ...
    FancytreeNode.prototype = /** @lends FancytreeNode# */{
        ...
        addChildren: function(children, insertBefore){
            ...
            if( this.tree.options.selectMode === 3 || this.tree.options.selectMode === 6){
...
//		this.debug("fixSelection3FromEndNodes()");
            _assert((this.tree.options.selectMode === 3 || this.tree.options.selectMode === 6), "expected selectMode 3, 6");
...
//				state = ( node.unselectable ) ? undefined : !!node.selected;
            ...
            //to avoid choise the parent element
            if (this.tree.options.selectMode === 6) {
                return;
            }
	 ...
	 * }});
        ...
        generateFormElements: function(selected, active, opts) {
            ...
            var nodeList,
                ...
                $result = $("#" + id),
                stopOnParents = (this.options.selectMode === 3 || this.options.selectMode === 6) && opts.stopOnParents !== false;
...
//            window.console.log("getNodeByKey(" + key + "): ", node.key);
                //node.key include the string value
...
//				parent.ul.appendChild(node.li);
            ...
            nodeRenderTitle: function(ctx, title) {
                ...
                if( opts.checkbox && node.hideCheckbox !== true && !node.isStatusNode() ) {
                    var checkboxUI = $(node.span).find('span.fancytree-checkbox').eq(0);
                    if(checkboxUI.length){
                        if(node.selected === true){
                            checkboxUI.addClass('fancytree-checkboxCheck');
                        } else {
                            checkboxUI.removeClass('fancytree-checkboxCheck');
                        }
                    }
                    let selectedClass = node.isSelected()?'fancytree-checkboxCheck':'';
                    if(aria){
                        ares.push("<span role='checkbox' class='fancytree-checkbox "+ selectedClass +"'></span>");
                    }else{
                        ares.push("<span class='fancytree-checkbox "+ selectedClass +"'></span>");
                    }
                }
...
//			tree.$container.attr("tabindex", "0");
                ...
                if( node.selected ){
                    ...
                    let checkboxUI = $(statusElem).find('span.fancytree-checkbox').eq(0);
                    if(checkboxUI.length){
                        checkboxUI.addClass('fancytree-checkboxCheck');
                    }
                }else if(aria){
...
//			this._fixSelectionState(node);
                ...
                } else if (opts.selectMode === 6){
                    node.selected = flag;
                    if(ctx.originalEvent) {
                        node.fixSelection3AfterClick();
                    }
                }
                node.selected = flag;
            ...
            treeLoad: function(ctx, source) {
                ...
                dfd = this.nodeLoadChildren(rootCtx, source).done(function(){
                    tree.render();
                    if( ctx.options.selectMode === 3 || ctx.options.selectMode === 6){
...
//          this._super( "_setOption", key, value );
            ...
            _bind: function() {
                ...
                tree.$container.on("focusin" + ns + " focusout" + ns, function(event){
                    ...
                    // if(opts.excludedClassesPrefix){
                    //
                    // }
                    if(opts.excludedClassesRegExp.test(event.target.className)){
                        event.preventDefault();
                        return false;
                    }
    ...
    $.extend($.ui.fancytree,
        /** @lends Fancytree_Static# */
        {
            ...
            getEventTarget: function(event){
                ...
                if(this().options.excludedClassesRegExp.test(tcn)){
                    event.preventDefault();
                    return false;
                }
...
;(function($, window, document, undefined) {
    ...
    $.ui.fancytree._FancytreeNodeClass.prototype.editEnd = function(applyChanges, _event){
        ...
        if( eventData.save ) {
            ...
            tree.options.renderNode.call(tree, {type: "renderNode"}, {node: node});
            node.setFocus();
        }else{
            if( eventData.isNew ) {
                node.remove();
...
//									node.setSelected();
                        ...
                        if( tree.options.selectMode === 3 || tree.options.selectMode === 6 ){
