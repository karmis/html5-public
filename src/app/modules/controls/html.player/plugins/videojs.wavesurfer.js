(function (root, factory) {
  return factory(videojs, require('wavesurfer.js'));
  ...
  videojs.Waveform = videojs.extend(VjsComponent,
    {
      ...
      hide: function() {
        this.surfer && $(this.surfer.mediaContainer).hide();
        this.waveReady = false;
      },
      show: function() {
        this.surfer && $(this.surfer.mediaContainer).show();
      },
      ...
      setupUI: function () {
        // customize controls
        this.player().bigPlayButton && this.player().bigPlayButton.hide();
        ...
        if (this.player().options_.controls) {
          ...
          this.player().controlBar && this.player().controlBar.show();
          ...
          // {
          //   this.player().controlBar.remainingTimeDisplay && this.player().controlBar.remainingTimeDisplay.hide();
          // }
          ...
          if (!this.liveMode) {
            this.player().controlBar.playToggle && this.player().controlBar.playToggle.hide();
          }
        }
        ...
        this.player().on('error', this.onWaveError.bind(this));
        ...
        this.player().on('resetWaveform', function(){
          // /*debugger*/;
        });
      ...
      startPlayers: function () {
        ...
        if (options.src !== undefined) {
          if (this.microphone === undefined) {
            ...
            this.player().loadingSpinner && this.player().loadingSpinner.show();
            ...
            this.player().loadingSpinner && this.player().loadingSpinner.hide();
      ...
      initialize: function (opts) {
        this.doInit(opts);
        var _this = this;
        $($(this.player().contentEl()).parent().find("iframe")[0].contentWindow).resize(function () {
          setTimeout(function () {
            // _this.doInit(opts);

            var controlBarHeight = _this.player().controlBar.height();
            if (_this.player().options_.controls === true && controlBarHeight === 0) {
            ...
            _this.refreshSurfer($(_this.player().contentEl()).height() - controlBarHeight)
            // _this.surfer.drawer.setHeight()
      ...
      doInit: function (opts) {
        ...
        if (opts.waveformHeight === undefined) {
          ...
          opts.height = $(this.player().contentEl()).height() - controlBarHeight;
        }
        else {
        ...
        this.player().surfer = this.surfer;
      },
      ...
      onWaveReady: function () {
        if (this.player() && this.player().el_) {
          ...
          // this.player().controlBar.progressControl && this.player().controlBar.progressControl.hide();
          ...
          this.player().controlBar.playToggle && this.player().controlBar.playToggle.show();

          // hide big play button
          $(this.player().el()).find('.imfx-big-play-btn').hide();
      ...
      onWaveSeek: function () {
        this.setCurrentTime();
        this.player().currentTime(this.surfer.getCurrentTime());
      },
      ...
      syncPlayback: function () {
        var playerPosition = this.player().currentTime();
        var wavePosition = this.surfer.backend.getCurrentTime();
        if (playerPosition != wavePosition) {
          /*debugger*/;
          this.surfer.backend.seekTo(playerPosition);
          this.surfer.backend.play();
      ...
      onScreenChange: function () {
        ...
        if (this.waveReady) {
          ...
          this.refreshSurfer(newHeight);

        }
      },

      refreshSurfer: function (newHeight) {

        if (this.surfer && this.surfer.drawer) {
          ...
          // set new height
          this.surfer.params.height = newHeight - 0;//this.player().controlBar.height();
      ...
      redrawBuffer: function() {
        try {
          ...
          console.log("Can not handle peaks - check if CORS is enabled. " + e.toString());
        }
      },
      ...
      onWaveError: function (error) {
        ...
        // commented because of recursive call: onWaveError - trigger player error - onWave error - etc
        // if (this.player().el_) {
  ...
  var wavesurferPlugin = function (options) {
    ...
    // and hide before load
    $(player.waveform.el()).hide();
  };
