...
(function ($, videojs, undefined) {
  ...
  function registerVideoJsMarkersPlugin(options) {
    ...
    function getPosition(marker) {
      ...
      if (dur === Infinity) {
        dur = window.liveDuration;
      }
      return setting.markerTip.time(marker) / dur * 100;
    }
    ...
    function registerMarkerTipHandler(markerDiv) {
      markerDiv.on('mouseover', function () {
        ...
        if (!!markerTip && !!marker.text) {
            markerTip.find('.vjs-tip-inner').text(setting.markerTip.text(marker));
            var markerPositionPercent = getPosition(marker)
            var controlBarWidth = videoWrapper.find(".vjs-progress-control").width();
            var markerPosition = getPosition(marker) * controlBarWidth / 100; // in px
            if (controlBarWidth - markerPosition < markerTip.width()/2) { // right position
                markerPositionPercent = (controlBarWidth - markerTip.width()/2) * 100 / controlBarWidth;
            } else if (markerPosition < markerTip.width()/2) {  // left position
                markerPositionPercent = (markerTip.width() * 100 / 2 ) / controlBarWidth;
            }
            // margin-left needs to minus the padding length to align correctly with the marker
            markerTip.css({
                "left": markerPositionPercent + '%',
                "margin-left": -parseFloat(markerTip.width()) / 2 + 'px',
                "visibility": "visible",
                "top": '8px' //videoWrapper.find('.vjs-thumbnail-holder').length ? '-100px' : '-40px'
            });
            videoWrapper.find('.vjs-control-bar').prepend(markerTip);
        }
      });

      markerDiv.on('mouseout', function () {
          !!markerTip && markerTip.css("visibility", "hidden");
          videoWrapper.find('.vjs-progress-holder').prepend(markerTip);
      });
    }
    ...
    // player.on("imfxloadedmedia", function () {
      if(player.componentContext.samePlayer) {
          return;
      }
        player.componentContext.samePlayer = true;
        initialize();
    });
