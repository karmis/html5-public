...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        var defaults = {
            ...
            hiddenFrozenColumnOverflow: true,
        ...
        var isObservedOnRenderComplete = false; //performance optimization flag
        ...
        function init() {
            ...
            if(options.fullWidthRows){
                columnsForFullWidth = $.extend(true, [], columns);
            }
            ...
            for (var i = 0; i < columns.length; i++) {
                // if(options.fullWidthRows && (typeof columns[i].width == 'undefined')){
                //     continue;
                // }
            ...
            $viewportTopR = $("<div id='slick-viewport-right' class='slick-viewport slick-viewport-top slick-viewport-right' tabIndex='0' hideFocus />").appendTo($paneTopR);
            ...
            MutationObserver.prototype.target = {};
            MutationObserver.prototype.config = {};


            MutationObserver.prototype.observePrevious = function () {
        ...
        function finishInitialization(pluginOptions) {
            if (!pluginOptions) {
                pluginOptions = {}
            }
            if (!initialized) {
                ...
                // @HACK
                options = $.extend({}, options, options.frozenColumn, pluginOptions);
                if (jqOld.fn.mousewheel && ( options.frozenColumn > -1 || hasFrozenRows )) {
                    $viewport
                ...
                $canvas
                    ...
                    .delegate(".slick-cell", "drop", handleDropCell)
                    .delegate(".slick-cell", "dragenter", handleDragEnterCell)
                    .delegate(".slick-cell", "dragleave", handleDragLeaveCell)
                    .delegate(".slick-cell", "dragstart", handleDragStartCell)
                    .delegate(".slick-cell", "mouseenter", handleMouseEnter)
                    .delegate(".slick-cell", "mouseleave", handleMouseLeave)

                ;
                if (options.multiAutoHeight){
        ...
        function getCommonViewportClientWidth() {
            return $viewportTopL[0].clientWidth + $viewportTopR[0].clientWidth;
        }
        ...
        function measureScrollbar() {
            ...
            var $c = $("<div style='position:fixed; top:-10000px; left:-10000px; width:100px; height:100px; overflow:scroll;'></div>").appendTo(cont);
            var dim = {
        ...
        function hasViewportScrolls() {
            return {
                v: viewportHasVScroll,
                h: viewportHasHScroll
            };
        }
        ...
        function createColumnHeaders() {
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                if (m.sortable) {
                    ...
                    if(m.id >= 0) {
                        header.addClass("slick-header-non-sortable");
                        header.append("<span class='slick-non-sort-indicator'/>");
                    }
                }
        ...
        function setFrozenOptions() {
            ...
            if (options.frozenRow > -1) {
                hasFrozenRows = true;
                frozenRowsHeight = getHeightForRows(options.frozenRow);
            ...
            if (jqOld.fn.mousewheel && ( options.frozenColumn > -1 || hasFrozenRows )) {
                $viewport
                    .unbind("mousewheel", handleMouseWheel);
                $viewport
                    .bind("mousewheel", handleMouseWheel);
            }
            else {
        ...
        function setOverflow() {
            if (options.hiddenFrozenColumnOverflow){
                $viewportTopL.css({
                    'overflow-x': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'hidden' : 'hidden' : ( hasFrozenRows ) ? 'hidden' : 'auto',
                    'overflow-y': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'hidden' : 'hidden' : ( hasFrozenRows ) ? 'scroll' : 'auto',
                });

                $viewportTopR.css({
                    'overflow-x': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'hidden' : 'auto' : ( hasFrozenRows ) ? 'hidden' : 'auto',
                    'overflow-y': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'scroll' : 'auto' : ( hasFrozenRows ) ? 'scroll' : 'auto'
                });

                if ( options.frozenColumn > -1 ){
                    if(!viewportTopRObserver) {
                        bindViewportTopRMutationObserver();
                    }
                }
            } else {
                //init overflow settings
                $viewportTopL.css({
        ...
        function bindViewportTopRMutationObserver() {
            // if(viewportTopRObserver instanceof MutationObserver) {
            //     return;
            // }

            var configObserver = { attributes: true,
                characterData: false,
                childList: false,
                subtree: false,
                ...
                , previousState = false
                , scrollHeight = scrollbarDimensions.height;

            viewportTopRObserver = {};

            viewportTopRObserver.canvObs = new MutationObserver(mcb);
            viewportTopRObserver.canvObs.target = $canvasTopR[0];
            viewportTopRObserver.canvObs.config = configObserver;
            viewportTopRObserver.canvObs.observe($canvasTopR[0], configObserver);

            viewportTopRObserver.vpObs = new MutationObserver(mcb);
            viewportTopRObserver.vpObs.target = $viewportTopR[0];
            viewportTopRObserver.vpObs.config = configObserver;
            viewportTopRObserver.vpObs.observe($viewportTopR[0], configObserver);

            function mcb(mutations) {
                if($viewportTopR[0].clientWidth < $viewportTopR[0].scrollWidth){
                    if(!previousState){
                        // viewportTopRObserver.disconnect();
                        previousState = true;
                        $viewportTopL.css({
                            'border-bottom': scrollHeight + 'px solid transparent'
                        });

                        //offset fix as long as last row is selected.
                        setTimeout(() => {
                            var st = $viewportTopR.scrollTop();
                            var _offset = st - $viewportTopR[0].scrollHeight + $viewportTopR[0].clientHeight + scrollHeight;
                            if (_offset > -scrollHeight) {
                                st += scrollHeight;
                            }
                            $viewportTopR.scrollTop(st);
                            $viewportTopL.scrollTop(st);
                        });
                        // viewportTopRObserver.observePrevious();
                    }
                } else {
                    if(previousState) {
                        // viewportTopRObserver.disconnect();
        ...
        function applyFullWidthRows(){
            var devideWidth, freeWidth, colWithWidth = 0, occupyWidth = 0,
                vpSL = $viewportTopL[0].offsetWidth - $viewportTopL[0].clientWidth,
                vpSR =  $viewportTopR[0].offsetWidth - $viewportTopR[0].clientWidth;

            if (!columnsForFullWidth.length){
                return;
            }

            if (options.allowBigFullWidthRows && typeof options.bigFullWidthRowsDelegate == 'function') {
                var _cols = options.bigFullWidthRowsDelegate();
                columnsForFullWidth = $.extend(true, [], _cols);
            }

            for (var i = 0; i < columnsForFullWidth.length; i++) {
                if (typeof columnsForFullWidth[i].width == 'number' && !columnsForFullWidth[i].calculatedWidth){
                    occupyWidth += columnsForFullWidth[i].width;
                    colWithWidth++;
                }
            }

            freeWidth = viewportW - vpSL - vpSR - occupyWidth - 1;

            if (freeWidth > 0){
                devideWidth = Math.ceil(freeWidth / (columnsForFullWidth.length - colWithWidth));

                for (var i = 0; i < columnsForFullWidth.length; i++) {
                    if (typeof columnsForFullWidth[i].width != 'number'){
                        columnsForFullWidth[i].width = devideWidth;
                        columnsForFullWidth[i].calculatedWidth = true;
                    } else if (options.allowBigFullWidthRows && columnsForFullWidth[i].calculatedWidth) {
                        if (columnsForFullWidth[i].width <= devideWidth) {
                            columnsForFullWidth[i].width = devideWidth;
                        } else {
                            freeWidth -= columnsForFullWidth[i].width;
                            colWithWidth++;
                            if (columnsForFullWidth.length > colWithWidth) {
                                devideWidth = Math.ceil(freeWidth / (columnsForFullWidth.length - colWithWidth));
                            } else {
                                break;
                            }
            ...
            var reRender = false;
            for (i = 0; i < columns.length; i++) {
                if (columns[i].rerenderOnResize && columns[i].width != columnsForFullWidth[i].width) {
                    reRender = true;
                }
                columns[i].width = columnsForFullWidth[i].width;
            }
        ...
        function applyColumnHeaderWidths() {
            ...
            for (var i = 0, headers = $headers.children(), ii = headers.length; i < ii; i++) {
                ...
                if (h[0].offsetWidth !== columns[i].width - headerColumnWidthDiff) {
        ...
        function setSortColumns(cols) {
            ...
            headerColumnEls.removeClass("slick-header-column-sorted").find(".slick-sort-indicator").removeClass("slick-sort-indicator-asc icons-up icons-down icon slick-sort-indicator-desc");
            ...
            $.each(sortColumns, function (i, col) {
                ...
                if (columnIndex != null) {
                    headerColumnEls.eq(columnIndex).addClass("slick-header-column-sorted").find(".slick-sort-indicator").addClass(
                        col.sortAsc ? "slick-sort-indicator-asc icons-up icon" : "slick-sort-indicator-desc icons-down icon");
                }
            });
        }
        ...
        function handleSelectedRangesChanged(e, ranges) {
            if(options.suppressSelection == true) {
                return;
            }
            selectedRows = [];
            var hash = {};
        ...
        function setColumns(columnDefinitions) {
            columns = $.extend(true, [], columnDefinitions);
        ...
        function getRowTop(row) {
            var heightObj = getRowHeight && getRowHeight(row);
            var counter = (heightObj && heightObj.counter) || 0;
            var unionOffset = (heightObj && heightObj.unionOffset) || 0;
            return options.rowHeight * (row - counter) - offset + unionOffset;
        }

        function getRowFromPosition(y) {
            var impliedValue = Math.floor((y + offset) / options.rowHeight);
            var result = isWithin(impliedValue);
            // console.log(result);
            return result;

            function isWithin(imVal){
                var impliedTop = getRowTop(imVal);
                var heightObj = getRowHeight && getRowHeight(imVal);
                var rowHeight = heightObj && heightObj.height || options.rowHeight;

                if (impliedTop <= y) {
                    if ((impliedTop + rowHeight) > y ){
                        return imVal;
                    } else {
                        return isWithin(imVal + 1);
                    }
                }
        ...
        function getRowHeight(rowIndex) {
            ...
            for (var key in rows){
                ...
                if(rows[key] && rows[key].customRowHeight){
                    unionOffset += rows[key].customRowHeight.height;
                    counter++;
                    rows[key].customRowHeight.unionOffset = unionOffset;
                    rows[key].customRowHeight.counter = counter;
                }
            }
            ...
            result.height = rows[rowIndex] && rows[rowIndex].customRowHeight  && rows[rowIndex].customRowHeight.height;

            return result;
        };

        function getHeightForRows(posNumber) {
            var summ = 0
                ,cacheLength = 0
                ,rowsCount = ((posNumber !== undefined) && (typeof posNumber == 'number')) ? posNumber : getDataLength();
            if (options.multiAutoHeight){
                for(var key in rowsCache){
                    summ += (rowsCache[key].customRowHeight) ? rowsCache[key].customRowHeight.height : options.rowHeight;
                    cacheLength++;
                }
                summ += options.rowHeight * (rowsCount - cacheLength);
            } else {
                summ = options.rowHeight * rowsCount;
            }
            return summ;
        }
        ...
        function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
            ...
            // var rowHeight = (data.getRowHeight && data.getRowHeight(row) && data.getRowHeight(row).height) || options.rowHeight;
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            // debugger
            // var cellHeight = (data.getRowHeight && data.getRowHeight(row) && data.getRowHeight(row).height) || options.rowHeight;
            var cellHeight = options.rowHeight;
            var cellHtml = "<div class='" + cellCss + "' style='height:"
        ...
        function continueObservers(){
            for(var i in observers){
                observers[i] && observers[i].observePrevious();
            }
        }

        function stopObservers(){
            for(var i in observers){
                observers[i] && observers[i].disconnect();
            }
        }
        ...
        function bindMutationObserver(){
            ...
            for (var i = 0; i < $canvas.length; i++){
                var observer = new MutationObserver(function(mutations) {
                    contentFitInRowsChecking(mutations, this);
                });

                observer.target = $canvas[i];
                observer.config = config;
                observer.observe($canvas[i], config);
        ...
        function contentFitInRowsChecking(mutations, context){
            if (!options.multiAutoHeight || isHiddenContainer()) {
                return;
            }

            // mutation is not nececcary parameter. It's use if this function MutationObserver callback
            var rowIndex, isContain = false, rowColumns, locDiff = 0, globDiff = 0;

            //we exclude parent nodes
            if (Array.isArray(mutations) && rowsCache[0] && rowsCache[0].rowNode[0]){
                for (var i = 0; i < mutations.length; i++){
                    if (mutations[i].target.contains(rowsCache[0].rowNode[0])) {
            ...
            for (var key in rowsCache){
                rowColumns = [];
                for (var i = 0; i< rowsCache[key].rowNode.length; i++) {
                    // if (Array.isArray(mutations)){
                    //     for (var j in mutations){
                    //         isContain = rowsCache[key].rowNode[i].contains(mutations[i].target);
                    //         if (isContain) {
                    //             mutations.splice(i,1);
                    //             break;
                    //         }
                    ...
                    // }
                    isContain = true;
                    rowColumns = rowColumns.concat(Array.from(rowsCache[key].rowNode[i].children));
                }

                locDiff = 0;
                if (isContain) {

                    locDiff = rowColumns.reduce(function (oldDiff, child) {
                        var firstChildClH = child.firstChild && child.firstChild.clientHeight || 0;
                        var curDiff = Math.max(child.scrollHeight, firstChildClH) - child.clientHeight;
                        if (curDiff > oldDiff) {
                            return curDiff;
                        }
                        return oldDiff;
                    ...
                    if (locDiff !== 0) {
                        context && (context instanceof MutationObserver)
                            ? context.disconnect()
                            : stopObservers();

                        if (rowsCache[key].customRowHeight) {
                            rowsCache[key].customRowHeight.height += locDiff;
                        } else {
                            rowsCache[key].customRowHeight = {
                                height: (rowsCache[key].rowNode[0].style.height.substr(0, rowsCache[key].rowNode[0].style.height.indexOf("px")) - 0) + locDiff
                            }
                        }

                        rowColumns.forEach(function (child) {
                            child.style.height = (child.style.height.substr(0, child.style.height.indexOf("px")) - 0) + locDiff + "px";
                        });

                        for (var i = 0; i< rowsCache[key].rowNode.length; i++) {
                            var rowN = rowsCache[key].rowNode[i];
                            rowN.style.height = (rowN.style.height.substr(0, rowN.style.height.indexOf("px")) - 0) + locDiff + "px";
                        }
                ...
                if (globDiff !== 0) {
                    for (var i = 0; i< rowsCache[key].rowNode.length; i++) {
                        var rowN = rowsCache[key].rowNode[i];
                        var prevRowsHeight = 0;         //toDo rechecking
                        for (var j = 0; j < key; j++) { // summ heights for prew rows
                            prevRowsHeight += rowsCache[j].customRowHeight ? rowsCache[j].customRowHeight.height : options.rowHeight;
                        }
                        // rowN.style.top = (rowN.style.top.substr(0, rowN.style.top.indexOf("px")) - 0) + globDiff - locDiff + "px";
                        rowN.style.top = prevRowsHeight + "px";
                    }
                }
            ...
            if (globDiff !== 0) {
                ...
                if (Array.isArray(mutations)){
                    contentFitInRowsChecking(mutations);
                } else{
                    contentFitInRowsChecking();
                }
                // resizeCanvas();
            }
            ...
            context && (context instanceof MutationObserver)
                ? context.observePrevious()
                : continueObservers();
        }

        function isHiddenContainer() {
            return !$container[0].offsetWidth && !$container[0].offsetHeight;
        }
        ...
        function getViewportHeight() {
            if (options.autoHeight) {
                viewportH = getHeightForRows(getDataLengthIncludingAddNew())
                ...
                var pt = parseFloat($.css($container[0], "paddingTop", true));
                var pb = parseFloat($.css($container[0], "paddingBottom", true));
        ...
        function resizeCanvas() {
            ...
            if (options.forceFitColumns) {
                autosizeColumns();
                if  (options.fullWidthRows){
                    console.log('Forcefitcolumn & fullWidthRows options can\'t work both with true value. ForceFitColumn have a more hight priority.');
                }
            } else {
                if  (options.fullWidthRows){
                    applyFullWidthRows();
        ...
        function updateRowCount() {
            ...
            viewportHasVScroll = !options.autoHeight && ( getHeightForRows(numberOfRows) > tempViewportH);
            ...
            for (var i in rowsCache) {
                if ((i >= l) && !(options.multiAutoHeight)) {
                    // incorrect behavior rendering last row
                    // removeRowFromCache(i);
                }
            }
            th = Math.max(getHeightForRows(), tempViewportH - scrollbarDimensions.height);
            ...
            if (h != oldH && options.autoHeight && !options.tileMode) {
                resizeCanvas();
            }
        ...
        function getRenderedRange(viewportTop, viewportLeft) {
            ...
            var sss = {
                top: 0,
                bottom: Math.min(getDataLengthIncludingAddNew() - 1, range.bottom),
                // bottom: getDataLengthIncludingAddNew()-1,
                leftPx: 0,
                // rightPx: range.rightPx
                rightPx: canvasWidth
            };
        ...
        function ensureCellNodesInRowsCache(row) {
            ...
            if (cacheEntry) {
                if (cacheEntry.cellRenderQueue.length) {
                    if(cacheEntry.rowNode && cacheEntry.rowNode.children){
                        var $childrenNodes = cacheEntry.rowNode.children('.slick-cell');
                        while (cacheEntry.cellRenderQueue.length) {
                            var columnIdx = cacheEntry.cellRenderQueue.pop();
                            cacheEntry.cellNodesByColumnIdx[columnIdx] = $childrenNodes[columnIdx];
                        }
                    }
        ...
        function renderRows(range) {
            ...
            // var rowsCacheLength = Object.keys(rowsCache).length;
            ...
            for (var i = 0, ii = rows.length; i < ii; i++) {
                if (( hasFrozenRows ) && ( rows[i] >= actualFrozenRow )) {
                    if (options.frozenColumn > -1) {
                        if (rows[i] === 0) {
                            rowsCache[rows[i]].rowNode = $()
                                .add($(x.firstChild).prependTo($canvasBottomL))
                                .add($(xRight.firstChild).prependTo($canvasBottomR));
                        } else {
                            rowsCache[rows[i]].rowNode = $()
                                .add($(x.firstChild).insertAfter(rowsCache[rows[i] - 1].rowNode[0]))
                                .add($(xRight.firstChild).insertAfter(rowsCache[rows[i] - 1].rowNode[1]));
                        }
                    ...
                    if (rows[i] === 0) {
                        rowsCache[rows[i]].rowNode = $()
                            .add($(x.firstChild).prependTo($canvasTopL))
                            .add($(xRight.firstChild).prependTo($canvasTopR));
                    } else {
                        rowsCache[rows[i]].rowNode = $()
                            .add($(x.firstChild).insertAfter(rowsCache[rows[i] - 1].rowNode[0]))
                            .add($(xRight.firstChild).insertAfter(rowsCache[rows[i] - 1].rowNode[1]));
                ...
                trigger(self.onRenderedRow, {rowId: rows[i]});
            }
        ...
        function updateRowPositions() {
            ...
            console.log('updateRowPositions');
        }
        ...
        function render() {
            ...
            if(options.suppressCleanup === false){
            ...
            postProcessFromRow = rendered.top;
            postProcessToRow = rendered.bottom;
            startPostProcessing();
            ...
            if(!isObservedOnRenderComplete){
                isObservedOnRenderComplete = true;
                setTimeout(function () {
                    isObservedOnRenderComplete = false;
        ...
        function handleMouseWheel(event, delta, deltaX, deltaY) {
            scrollTop = Math.max(0, $viewportScrollContainerY[0].scrollTop - (getHeightForRows(deltaY)));
        ...
        function _handleScroll(isMouseWheel) {
            ...
            if (hScrollDist) {
                ...
                $viewportScrollContainerX[0].scrollLeft = $headerScrollContainer[0].scrollLeft = scrollLeft;
                $topPanelScroller[0].scrollLeft = $headerRowScrollContainer[0].scrollLeft = scrollLeft;
        ...
        function handleDropCell(e, dd) {
            trigger(self.onDropCell, dd, e);
        }
        function handleDragEnterCell(e, dd) {
            trigger(self.onDragEnterCell, dd, e);
        }
        function handleDragLeaveCell(e, dd) {
            trigger(self.onDragLeaveCell, dd, e);
        }
        function handleDragStartCell(e, dd) {
            trigger(self.onDragStartCell, dd, e);
        }
        ...
        function getFrozenRowOffset(row) {
            var offset =
                ( hasFrozenRows )
                    ...
                    ? ( row >= actualFrozenRow )
                        ? ( h < viewportTopH )
                            ? (getHeightForRows(actualFrozenRow))
                            : h
                        : 0
                    : ( row >= actualFrozenRow )
                        ? frozenRowsHeight
        ...
        function getCellFromEvent(e) {
            ...
            if (options.tileMode){
                var row = getRowFromNode([$cell[0].parentElement]);
            } else {
        ...
        function setFocus() {
            if (!options.enableNativeGridFocus)
                return;

            var isSafari = (getNameBrowser() === 'Safari');
            if (isSafari) {
                //Safari decision
                setTimeout(()=>{
        ...
        function scrollRowIntoView(row, doPaging) {
            ...
            var rowAtTop = getHeightForRows(row);
            var rowAtBottom = getHeightForRows(row + 1)
                - viewportScrollH
                + (viewportHasHScroll ? scrollbarDimensions.height : 0);

            // need to page down?
            if (getHeightForRows(row + 1) > scrollTop + viewportScrollH + offset) {
                scrollTo(doPaging ? rowAtTop : rowAtBottom);
                render();
            }
            // or page up?
            else if (getHeightForRows(row) < scrollTop + offset) {
        ...
        function scrollRowToTop(row) {
            scrollTo(getHeightForRows(row));
            render();
        }

        function scrollToYAsix(y) {
            scrollTo(y);
            render();
        }
        ...
        function scrollPage(dir) {
            ...
            scrollTo(getHeightForRows((getRowFromPosition(scrollTop) + deltaRows)));
            render();
        ...
        function gotoCell(row, cell, forceEdit) {
            ...
            // if selecting the 'add new' row, branding editing right away
        ...
        $.extend(this, {
            ...
            "onDropCell": new Slick.Event(),
            "onDragEnterCell": new Slick.Event(),
            "onDragLeaveCell": new Slick.Event(),
            "onDragStartCell": new Slick.Event(),
            "onSelectedRowsChanged": new Slick.Event(),
            "onCellCssStylesChanged": new Slick.Event(),
            "onRenderedRow": new Slick.Event(),
            ...
            "contentFitInRowsChecking": contentFitInRowsChecking,
            "isHiddenContainer": isHiddenContainer,
            "setOverflow":setOverflow,
            "getRowTop": getRowTop,
            "getCommonViewportClientWidth": getCommonViewportClientWidth,
            "getScrollbarDimensions": getScrollbarDimensions,
            "hasViewportScrolls":hasViewportScrolls,
