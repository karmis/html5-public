...
import {AdvancedCriteriaControlDateTypedDirtyValueType, AdvancedCriteriaControlDateTypedType} from "./types";
const epochTicks = 621355968000000000;
const ticksPerMillisecond = 10000;

@Component({
    selector: 'advanced-criteria-control-datetyped',
    templateUrl: 'tpl/index.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.Default
})
export class IMFXAdvancedCriteriaControlDateTypedComponent {
    ...
    @ViewChild('friendlySelector', {static: false}) private friendlySelector: IMFXControlsSelect2Component; // select for interval type
    ...
    private modeAbs: boolean = false; // mode (absolute/relative)
    // available intervals
    private intervals = [
        {id: 'd', text: 'Days'},
        {id: 'h', text: 'Hours'}
    ];
    private customSorter = (a, b) => a.text.localeCompare(b.text);
    private friendlyDates = [
        {id:0, text:'Custom', order: 0},
        {id:1, text:'Today', order: 1},
        {id:2, text:'Yesterday', order: 2},
        {id:3, text:'Last Hour', order: 3},
        {id:4, text:'Last 7 Days', order: 4},
        {id:5, text:'Last 30 Days', order: 5},
        {id:6, text:'Tomorrow', order: 6},
        {id:7, text:'Next Hour', order: 7},
        {id:8, text:'Next 7 Days', order: 8},
        {id:9, text:'Next 30 Days', order: 9}];
    ...
    ngAfterViewInit() {
        ...
        if (critVal && critVal.dirtyValue && critVal.dirtyValue.mode) {
            ...
            this.inputUI.nativeElement.value = 0;
            this.friendlySelector.setSelected(this.selectedFormat);
    ...
    setValueByMode(absMode: boolean = null, dirtyValue: any = null): boolean {
        this.cdr.detach();
        const critVal = this.data.criteria.data.value;//parseInt(.split("|")[1]);
        if (critVal && critVal.value !== undefined && critVal.value !== '') {
            this.selectedFormat = (<any>critVal.value).split("|")[2];
            if(!this.selectedFormat) {
                this.selectedFormat = '1';
            }
             critVal.value = (<any>critVal.value).split("|")[1];
        }
        ...
        if (this.modeAbs === false) {
            if (dirtyValue) { // restore from dirtyVal (recent search)
                ...
                this.selectedFormat = dirtyValue.mode.selectedFormat;
            ...
            this.type = this.selectedInterval === 'd' ? 2 : 3;
            if (this.selectorUI) {
                this.selectorUI.setSelected(this.selectedInterval);
            }
            if (this.friendlySelector) {
                this.friendlySelector.setSelected(this.selectedFormat);
            }
            if(this.selectorUI || this.friendlySelector)
                this.transferData();
                return true;
        }
        else {
            this.type = this.modeAbs === true ? 1 : 2;
    ...
    transferData() {
        ...
        this.transfer.onSelectValue(<AdvancedSearchDataFromControlType>{
            value: this.type + "|" + valueForRequest + (this.type > 1 ? "|" + this.selectedFormat : ""),
            dirtyValue: <AdvancedCriteriaControlDateTypedDirtyValueType>{
            ...
            humanValue: this.getHumanValueTyped(this.modeAbs ? this.dtpUI.getValue() : this.inputUIVal)
        });
    }
    ...
    getHumanValue(val) {
        ...
        if (this.modeAbs) {
            ...
            let format = this.translate.instant('common.date_format') || 'dd.MM.yyyy H:mm';
    ...
    getHumanValueTyped(val) {
        ...
        if (this.modeAbs) {
            ...
            let datePipe = new DatePipe(locale);
            // datePipe = new DatePipe((<any>window).navigator.userLanguage || (<any>window).navigator.language);
            let format = this.translate.instant('common.date_format_long') || 'dd.MM.yyyy';
            res = datePipe.transform(val, format);
        } else {
            if(this.selectedFormat === '0')
                res = val + ' ' + this.intervals[this.selectedInterval === 'h' ? 1 : 0].text;
            else
                res = this.friendlyDates.filter((x)=>{return x.id.toString() == this.selectedFormat})[0].text;
        }
    ...
    onSelectType() {
        this.selectedFormat = this.friendlySelector.getSelected();
        if(this.selectedFormat === '0') {
            this.showCustom = true;
            this.selectorUI.setSelected(this.selectedInterval);
            this.inputUI.nativeElement.value = 0;
            ...
            if(this.selectedFormat === '3' || this.selectedFormat === '7') {
                this.selectedInterval = 'h';
            }
            else {
                this.selectedInterval = 'd';
            ...
            switch (this.selectedFormat) {
                case '1':
                    ...
                    break;
                case '2':
                    this.inputUI.nativeElement.value = -1;
                    break;
                case '3':
                    this.inputUI.nativeElement.value = -1;
                    break;
                case '4':
                    this.inputUI.nativeElement.value = -7;
                    break;
                case '5':
                    this.inputUI.nativeElement.value = -30;
                    break;
                case '6':
                    this.inputUI.nativeElement.value = 1;
                    break;
