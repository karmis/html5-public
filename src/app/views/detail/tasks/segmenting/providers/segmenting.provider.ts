...
export class SegmentingProvider extends DetailProvider {
    saveSegmenting: Subject<any> = new Subject();
    ...
    @Output() afterSavedSegmenting: EventEmitter<any> = new EventEmitter<any>();
    ...
    updateAndSaveMediaItems(data): Observable<Subscription> {
        return new Observable((observer: any) => {
            if (data == false) {
                ...
                let message = this.translate.instant('segmenting.invalid_save');
    ...
    save(): Observable<Subscription> {
        return new Observable((observer: any) => {
            ...
            this.config.options.service.save(this.getDetailId(), saveObj).subscribe((res: any) => {
                this.afterSavedSegmenting.emit(res);
                observer.next(res);
                let message = this.translate.instant('segmenting.success_save');
                this.notificationRef.notifyShow(1, message);
            }, (err) => {
                observer.error(err);
                let message = this.translate.instant('segmenting.error_save');
                this.notificationRef.notifyShow(2, message);
            }, () => {
                observer.complete();
            });
