...
import { ValidFields } from "../segmenting/constants/valid.fields";
...
import { ImfxVersionSegmentsTabComponent } from '../../../../modules/search/detail/components/version.segments.tab.component/imfx.version.segments.tab.component';
import { timelineConfigGroup } from '../../../../utils/imfx.common';

declare var GoldenLayout: any;

@Component({
    selector: 'golden-segmenting-layout',
...
export class GLSegmentingComponent extends GLComponent {
    @Input() onSaveSegmenting: Subject<any>;
    @Input() videoInfo: any;
    @Input() onErrorSaveSegmenting: Subject<any>;
    @Output() onUpdateMediaItems: EventEmitter<any> = new EventEmitter();
    private layoutModel: LayoutManagerModel;
    private layoutType: LayoutType = LayoutType.Segmenting;
    mediainfoComponent: any = null;
    private eventsComponent: any = null;
    private segmentsComponent: any = null;
    private versionSegmentsComponent: any = null;
    ...
    mediaListComponent: {compRef: {instance: SimpleListComponent, _component: SimpleListComponent}} = null;
    ...
    ValidateAndUpdateTabs(status = ''): Observable<Subscription> {
        return Observable.create((observer) => {
            ...
            if (status === 'COMPLETED' && !this.isValidCustomMetadata()) {
                this.notificationRef.notifyShow(2, this.translate.instant('segmenting.custom_metadata_invalid'));
                return;
            }
            ...
            if(this.customMetadataInEdit && status === 'COMPLETED') {
                this.notificationRef.notifyShow(2, this.translate.instant('segmenting.custom_metadata_in_edit'));
                observer.next(false);
                observer.complete();
            }
            else if (nonValidTimecodeTabs.length !== 0) {
                this.notificationRef.notifyShow(2, this.translate.instant('segmenting.check_timecodes'));
                observer.next(false);
                observer.complete();
            } else {
                let valid = this.allValidTabs.filter(el => {
                    ...
                    return el.isValid == false;
                }).length ? false : true; // if there is non valid tabs
    ...
    ngOnInit() {
        this.onSaveSegmenting.subscribe(res => {
            // save locators
        ...
        this.onErrorSaveSegmenting.subscribe(res => {
            let id = 0;
            this.config.moduleContext.mediaItems.forEach((el, ind) => {
                if (el.ID == this.config.options.file['ID']) {
                    id = ind;
        ...
        if (this.config.moduleContext.afterSavedSegmenting) {
            this.config.moduleContext.afterSavedSegmenting.subscribe(res => {
                debugger;
                let index = 0;
                this.config.moduleContext.mediaItems.forEach((el, ind) => {
                    if (el.ID == this.config.options.file['ID']) {
                ...
                this.mediaListComponent && this.mediaListComponent.compRef.instance.cdr.detectChanges();
        ...
        if (model.actualModel && model.actualModel.Layout) {
            ...
            if (state) {
                ...
                if (this.layoutModel && this.layoutModel.Layout) {
                    ...
                    this.layoutConfig = JSON.parse(LayoutManagerDefaults.Segmenting);
                }
            } else {
                this.layoutModel = model.defaultModel;
                this.layoutConfig = JSON.parse(LayoutManagerDefaults.Segmenting);
    ...
    addMediaTagging(self) {
        this.layout.registerComponent('Tagging', (container, componentState) => {
            ...
            let compRef = this.viewContainer.createComponent(factory);
            let provider = this.injector.get(SegmentingProvider);
    ...
    createNewTabsAfterInit() {
        ...
        let segmentingProvider = this.injector.get(SegmentingProvider);
        for (var i = 0; i < this.allValidTabs.length; i++) {
            ...
            if (buf.length === 0) {
                ...
                //     }
                //     tab.isValid = segmentingProvider.getValidation(this.config.options.file, obj);
                // }
                if (ValidFields[name]) {
                    let obj = {};
    ...
    addMediaList(self) {
        ...
        this.layout.registerComponent('MediaItems', (container, componentState) => {
            ...
            self.config.moduleContext.mediaItems.forEach(el => {
                ...
                el._isVersionSegmentsAdded = false;
            });
            ...
            segmentingProvider.selectedMediaItem.next(self.config.moduleContext.mediaItems[0]);
            ...
            compRef.instance.onSelect.subscribe(data => {
                ...
                segmentingProvider.selectedMediaItem.next(data.file);
                if (data.file.ID != self.config.options.file.ID) {
                    ...
                    self.config.options.params = segmentingProvider.setVideoBlock(data.file);
                    this.versionSegmentsComponent.compRef.instance.cdr.detectChanges();
                        ...
                        (resp) => {
                            ...
                            if (currentLoggerSeries.length === 0) { // if there is no loading series for current item
                                if (self.config.options.file.DFILE_LINK_GUID != '' && self.config.options.file.DFILE_LINK_GUID != null) {
                                    segmentingProvider.loadTagging(self.config.options.file.DFILE_LINK_GUID)
                            ...
                            segmentingProvider.loadSubtitles(self.config.options.file.ID)
                                .pipe(takeUntil(self.destroyed$))
     ...
     * Add segments tab
     */
    addVersionSegments(self) {
        this.layout.registerComponent('VersionSegments', (container, componentState) => {
            this.translateTitle(container, '.version_segments');
            let factory = this.componentFactoryResolver.resolveComponentFactory(ImfxVersionSegmentsTabComponent);
            let compRef = this.viewContainer.createComponent(factory);
            let goldenRef = this;
            container.getElement().append($(compRef.location.nativeElement));
            container['compRef'] = compRef;
            ...
            let segmentingProvider = this.injector.get(SegmentingProvider);
            const fileForSegment = _.cloneDeep(self.config.options.file);
            const file = self.config.options.file;
            fileForSegment.Segments = segmentingProvider.versions[0].Segments;
            compRef.instance['config'] = {
                file: fileForSegment,
                typeDetailsLocal: self.config.options.typeDetailsLocal,
                typeDetails: self.config.options.typeDetails,
            ...
            compRef.instance.copyToMediaSub.subscribe(tc => {
                file.Segments.push(..._.cloneDeep(segmentingProvider.versions[0].Segments).map(el => {
                    return {
                        ...
                        "TYPE": el.TYPE,
                        "ID": 0,
                        "MIID": fileForSegment.ID,
                        "PAR_TYPE": el.PAR_TYPE,
                        "TimecodeFormat": el.TimecodeFormat,
                        "customId": Math.random(),
                        "videoSom": el.videoSom,
                        "videoEom": el.videoEom,
                        "timecodesNotValid": false
                    }
                }))
                file.Segments.map((el, i) => {
                    el.SQ_NUM = i + 1;
                    return el
                });
                segmentingProvider.mediaItems.find(el => el.id === file.id).Segments = file.Segments;
                const found = self.config.moduleContext.mediaItems.find(el => el.id === fileForSegment.id);
                // found['_isVersionSegmentsAdded'] = true;
                this.mediaListComponent.compRef.instance.setItems(self.config.moduleContext.mediaItems);
                this.selectedMediaItem = {
                    file: self.config.moduleContext.mediaItems[0]
                }
                this.segmentsComponent && this.segmentsComponent.compRef.instance.updateSlickData();
                // compRef.instance.copyToMediaDisBtn = () => true;
            });

            compRef.instance.copyToMediaDisBtn = () => {
                let data = this.storageService.retrieve('permissions');
                return this.config.moduleContext.taskFile.TSK_STATUS !== JobStatuses.INPROG || this.config.moduleContext.taskFile.LOCKED_BY !== data.FullName || !this.selectedMediaItem.file.IsGangedMain;
            }
            self.versionSegmentsComponent = container;
            compRef.changeDetectorRef.detectChanges();
        });
    }
    ...
    addMediaInfoTab(self) {
        this.layout.registerComponent('MediaInfo', (container, componentState) => {
            ...
            // }
            // let segmentingProvider = this.injector.get(SegmentingProvider);
            // self.allValidTabs.filter( el => {return el.tTitle == container.title} )[0].isValid = segmentingProvider.getValidation(this.config.options.file, obj);
            this.layout.on('setReadOnly', function (readOnly) {
                compRef.instance.onRefresh.next({file: self.config.options.file, readOnly: readOnly});
            });
            compRef.changeDetectorRef.detectChanges();
    ...
    getReadOnlyModeForTab(file) {
        this.versionSegmentsComponent && this.versionSegmentsComponent.compRef.instance.cdr.detectChanges();
        let data = this.storageService.retrieve('permissions');
