...
import { FolderSlickGridProvider } from './folder.slick.grid.provider';

@Injectable()
export class FolderViewsProvider extends ViewsProvider {
    config: ViewsConfig;
    ...
    getCustomColumns() {
        ...
        columns.unshift({
            ...
            header: {
                buttons: [{
                    showOnHover: false,
                    buttonHtml: '<i class="icons-basket icon" id="basket-icon-folders"></i>',
                    tooltip: "Add all items to Basket",
                    handler: (e) => {
                        const wrapper = $(e.target);
                        wrapper.fadeTo("slow", 0.1);
                        wrapper.attr('title', 'Saving...');
                        (this.config.componentContext.slickGridComp.provider as FolderSlickGridProvider).addAllToBasket()
                            .subscribe(() => {
                                wrapper.fadeTo("slow", 0.6)
                                wrapper.attr('title', 'Add all items to Basket');
