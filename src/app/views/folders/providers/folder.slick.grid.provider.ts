...
import { ChangeCustomStatusComponent } from "../comp/change-custom-status/change-custom-status.comp";
...
export class FolderSlickGridProvider extends SlickGridProvider {
    ...
    changeStatus(status) {
        ...
        let modal: IMFXModalComponent = this.modalProvider.showByPath(
            ...
            {
                title: this.translate.instant('media_status_modal.title') + title,
                size: 'md',
                position: 'center',
                footer: 'cancel|save'
    ...
    onChangeCustom() {
        ...
        modal.load().then((modal: ComponentRef<ChangeCustomStatusComponent>) => {
            ...
            modalContent.loadData(data, 'media', this);
        });
    ...
    addAllToBasket(): Observable<void> {
        // this.basketService.clearBasket();
        return new Observable((observer) => {
            const itemsToSave: any[] = [];
            this.getClearDataAsArray().forEach((item) => {
                if (!this.basketService.hasItem(item)) {
                    this.basketService.addToBasket(item, 'Media', false);
                    itemsToSave.push(item);
                }
            })

            this.serverStorage.store('media.basket.data', {items: itemsToSave}).subscribe(() => {
                const msg = this.translate.instant('basket.added_to_basket_count', {count: this.getData().length});
                this.notificationService.notifyShow(1, msg, true, 1200);
                observer.next();
                observer.complete();
            });
